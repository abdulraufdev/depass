...
class EncryptionService {
  ...
  encrypt_pkg.Key _deriveKey(String password) {
    ...
    for (int i = 0; i < transformedSalt.length; i++) {
      int combined =
          (transformedSalt[i] ^ appSeedBytes[i % appSeedBytes.length]) & 0xFF;
  ...
  Future<EncryptionResult> encryptTextAdvanced({
    ...
    try {
      ...
      final encrypter = encrypt_pkg.Encrypter(
        encrypt_pkg.AES(key, mode: encryptMode),
      );

      final encrypted = encrypter.encrypt(plaintext, iv: iv);
      return EncryptionResult(encryptedText: encrypted.base64, iv: ivString);
    } catch (e) {
  ...
  static const String _seedPhraseKey = 'deu4309h3kQwFkkRsD),dsdekdkjd';

  Future<String> generateSeedPhrase() async {
    final mnemonic = bip39.generateMnemonic(strength: 256);
    await _secureStorage.write(key: _seedPhraseKey, value: mnemonic);
    return mnemonic;
  }
}
